...
namespace BLD
{
    public class BLDTexture : BaseDisposable
    {
        [System.Serializable]
        ...
        public BLDTexture() { model = new Model(); }

        public static IEnumerator FetchFromComponent(IParcelScene scene, string componentId,
            System.Action<Texture2D> OnFinish)
        {
            yield return FetchTextureComponent(scene, componentId, (bldTexture) => { OnFinish?.Invoke(bldTexture.texture); });
        }

        public static IEnumerator FetchTextureComponent(IParcelScene scene, string componentId,
            System.Action<BLDTexture> OnFinish)
        {
            if (!scene.disposableComponents.ContainsKey(componentId))
            {
                Debug.Log($"couldn't fetch texture, the BLDTexture component with id {componentId} doesn't exist");
                yield break;
            }

            BLDTexture textureComponent = scene.disposableComponents[componentId] as BLDTexture;

            if (textureComponent == null)
            {
                Debug.Log($"couldn't fetch texture, the shared component with id {componentId} is NOT a BLDTexture");
                yield break;
            }
        ...
        public override IEnumerator ApplyChanges(BaseModel newModel)
        {
            ...
            if (texture == null && !string.IsNullOrEmpty(model.src))
            {
                ...
                if (isBase64)
                {
                    ...
                    {
                        Debug.LogError($"BLDTexture with id {id} couldn't parse its base64 image data.");
                    }
