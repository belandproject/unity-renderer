...
{
    public class BLDVideoTexture : BLDTexture
    {
        public static bool VERBOSE = false;
        public static Logger logger = new Logger("BLDVideoTexture") { verboseEnabled = VERBOSE };
        ...
        public override IEnumerator ApplyChanges(BaseModel newModel)
        {
            ...
            {
                BLDVideoClip bldVideoClip = scene.GetSharedComponent(lastVideoClipID) as BLDVideoClip;

                if (bldVideoClip == null)
                {
                    logger.Error( "Wrong video clip type when playing VideoTexture!!");
                    yield break;
                }

                Initialize(bldVideoClip);
            }
        ...
        private void Initialize(BLDVideoClip bldVideoClip)
        {
            string videoId = (!string.IsNullOrEmpty(scene.sceneData.id)) ? scene.sceneData.id + id : scene.GetHashCode().ToString() + id;
            texturePlayer = new WebVideoPlayer(videoId, bldVideoClip.GetUrl(), bldVideoClip.isStream, videoPluginWrapperBuilder.Invoke());
        ...
        private void CalculateVideoVolumeAndPlayStatus()
        {
            ...
            if (attachedMaterials.Count > 0)
            {
                using (var iterator = attachedMaterials.GetEnumerator())
                {
                    while (iterator.MoveNext())
                    {
                        ...
                        {
                            isVisible = true;
                            var entityDist = materialInfo.Value.GetClosestDistanceSqr(BLDCharacterController.i.transform.position);
                            ...
                            if (minDistance <= BLD.Configuration.ParcelSettings.PARCEL_SIZE * BLD.Configuration.ParcelSettings.PARCEL_SIZE)
                                break;
                        }
                    }
                }
            ...
            if (isVisible)
            {
                ...
                float sqrParcelDistance = BLD.Configuration.ParcelSettings.PARCEL_SIZE * BLD.Configuration.ParcelSettings.PARCEL_SIZE * 2.25f;
        ...
        void OnEntityRemoved(IBLDEntity entity) { isPlayStateDirty = true; }
        ...
        private void OnEntityAttachedMaterial(IBLDEntity entity) { entity.OnShapeUpdated += OnEntityShapeUpdated; }

        private void OnEntityDetachedMaterial(IBLDEntity entity)
        {
            if (texturePlayer != null)
                texturePlayer.Pause();
        ...
        private void OnEntityShapeUpdated(IBLDEntity entity) { isPlayStateDirty = true; }
        ...
        struct MaterialComponent : MaterialInfo
        {
            ...
            bool IsEntityVisible(IBLDEntity entity)
            {
